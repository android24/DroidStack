...
不需要第一次就把所有历史细节读完。根据当前目标选择路线：

| 路线 | 阅读顺序 | 适合场景 |
| --- | --- | --- |
| 快速路线 | 交互轨道 -> [3.0.7 变化驱动力](#_3-0-7-到底是什么推动框架持续变化) -> [3.0.11 稳定主流程](#_3-0-11-变化很多-但主流程从未消失) -> [检查点](#_3-0-12-本节检查点) | 想尽快进入请求管线与 Demo |
| 深读路线 | 按 3.0.1 至 3.0.13 顺序阅读，并完成历史侦探 | 想理解每代框架为何形成不同设计重心 |

快速路线负责建立问题地图，深读路线负责补全历史证据。两条路线最终都会进入同一条图片请求主流程。
