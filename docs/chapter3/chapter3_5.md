...
只把 `Builder、Strategy、Factory` 填进表格，仍然没有理解设计模式。真正的判断标准是：它把哪一类变化隔离了，运行时有哪些对象参与，去掉这个模式后哪部分代码会开始膨胀。
...
[<- 上一节：3.4 生命周期、列表复用与资源回收](./chapter3_4.md) | [继续：3.6 框架比较、工程选型与综合实验 ->](./chapter3_6.md)
