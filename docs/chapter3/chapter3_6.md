# 第 3.6 节：框架比较、工程选型与综合作业设计

理解原理之后，选型不应变成“新项目用 Coil、老项目用 Glide”的口号。本节把业务需求映射到框架能力，并把前面学到的机制整理成一份可分阶段完成的综合作业蓝图。下一节再用现成的 Web 实验台和 Android Demo 提供第一批可运行证据。
...
## 3.6.6 综合作业蓝图：Image Pipeline Lab

综合作业不是再写一遍 `AsyncImage`，而是逐步完成一个可观察、可故障注入的商品图片实验室。

本节定义最终目标和验收标准，不表示配套 Demo 已经一次性实现全部要求。[第 3.7 节](./chapter3_7.md)提供可以立即操作的交互实验台、挑战答案和 Coil / Glide Android Demo；读者再以它们为起点完成列表、详情、延迟注入和完整观测。
...
| 能力 | Web 实验台 | Android Demo | 综合作业继续完成 |
| --- | --- | --- | --- |
| 图片与请求阶段可视化 | 已提供 | Coil / Glide 日志 | 增加统一 Trace 面板 |
| 多级缓存与版本 Key | 已模拟 | 已提供基础操作 | 对比进程重启与磁盘命中 |
| Bitmap 内存估算 | 已提供 | 已提供尺寸估算 | 增加设备内存采样 |
| 列表竞态 | 已动画复现 | Race Lab 已复现 | 改造成真实 RecyclerView 100 条列表 |
| 失败分类 | 404、超时、损坏、格式不支持 | 当前提供 404 | 增加测试网络层和四类故障 |
| 设计模式定位 | 已提供模式观察器 | 可进入 Coil / Glide 源码 | 按断点记录真实运行对象 |
| 详情页、失败率、完整监控 | 未提供 | 未提供 | 综合作业目标 |

这张表是课程契约：标记为“已提供”的能力可以立即验证；标记为“继续完成”的能力是作业，不应在运行现有 Demo 时被误认为已经存在。
...
某团队有 70 个 XML 页面、4 个复杂 RecyclerView、现成 Glide ModelLoader 和两年维护经验；新模块开始使用 Compose，但只占应用的 15%。现在有人提议“为了现代化，三个月内全部迁移 Coil”。

先完成三步判断：

1. 写出至少两个不可妥协条件；
2. 给 Compose 体验、迁移成本、团队经验和 View 列表能力设置权重；
3. 分别给出“全量迁移、存量治理、新模块局部引入”的证据与风险。

<details>
<summary>展开参考决策</summary>

在没有性能缺陷、维护中断或扩展阻塞证据时，全量迁移不是默认答案。更稳妥的候选方案是治理现有 Glide 封装与监控，同时在边界清晰的新 Compose 模块验证 Coil；只有局部实验能证明收益覆盖迁移和双栈成本后，再扩大范围。答案的关键不是选择了哪个框架，而是约束、权重和证据是否可复查。
