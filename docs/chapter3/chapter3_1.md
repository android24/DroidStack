...
> 开局预测：同一页面同时请求两次完全相同的图片时，是执行两次网络下载，还是两个调用者等待同一个任务？先写下答案，再阅读第 3.1.5 节验证；读完主流程后可到 [请求管线实验](./chapter3_7.md?lab=cache#_3-7-2-交互实验台) 观察各阶段如何被命中或跳过。
...
[<-- 上一节：3.0 从 Universal Image Loader 到 Coil](./chapter3_0.md) | [进入 Demo：观察请求管线](./chapter3_7.md?lab=cache#_3-7-2-交互实验台) | [继续：3.2 缓存系统与缓存 Key ->](./chapter3_2.md)
